...
exports.smsg = (conn, citel, store) => {
  ...
  if (citel && citel.msg && citel.msg.url) citel.download = () => conn.downloadMediaMessage(citel.msg)
...
*/
  ...
  citel.senddoc = (doc, type, id = citel.chat, option = {
    mentions: [citel.sender], filename: '111', mimetype: type,
    externalAdRepl: {
      title: '11111123',
      body: ' ',
      thumbnailUrl: ``,
...
exports.checkPhoneNumber = (text) => {
  const phonePattern = /\b\d{3}[-.\s]?\d{3}[-.\s]?\d{2}[-.\s]?\d{2}\b/g;
  return phonePattern.test(text);
}

exports.sendRequestGPT = async ({ prompt, ver }) => {
  try {
    const res = await axios.post('https://iam.api.cloud.yandex.net/iam/v1/tokens', { yandexPassportOauthToken: 'y0_AgAAAABaeIcpAATuwQAAAAEDJWX4AAD-aBmeFoFHfrgLN9UU9rgppcCujQ' });
    ...
    if (ver === 'speech') {
      const audioData = fs.readFileSync(prompt);
      ...
      const url = `https://stt.api.cloud.yandex.net/speech/v1/stt:recognize?${params}`;
      ...
      try {
        ...
        if (!decodedData.error_code) {
          ...
          throw new Error(`Ошибка при распознавании речи: ${decodedData.error_code}, ${decodedData.error_message}`);
        }
      } catch (error) {
        throw new Error(`Ошибка при распознавании речи: ${error.message}`);
      }
    } else {
      const apiUrl = "https://llm.api.cloud.yandex.net/foundationModels/v1/completion";
      const bodyData = {
        modelUri: "gpt://b1gr6ioajp8k1euqr75r/yandexgpt/latest",
        completionOptions: {
          stream: false,
          temperature: 0.6,
          maxTokens: 8000,
      ...
      if (!response.data.result || !response.data.result.alternatives || !response.data.result.alternatives[0].message || !response.data.result.alternatives[0].message.text) {
        throw new Error('Text completion data not found in the response.');
      }

      return response.data.result.alternatives[0].message.text;
    }
  } catch (error) {
    console.error('Error during GPT request:', error.message);
    throw error;
  }
}

exports.getSessionList = async () => {
  ...
  try {
    const [rows] = await connection.execute('SELECT session FROM auth');
    const uniqueSessions = new Set(rows.map(row => row.session));
